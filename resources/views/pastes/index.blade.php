...
<div>
    ...
    <div class="relative overflow-hidden rounded-2xl bg-gray-900 mb-10">
        ...
        <div class="relative px-6 py-16 sm:px-12 sm:py-20 lg:py-24 text-center">
            ...
            <div class="mt-8">
                <a href="{{ route('pastes.create') }}" class="btn btn-primary btn-lg shadow-xl">
    ...
    @if($pastes->count() > 0)
        <div class="mb-6">
            ...
            <p class="mt-1 text-sm text-gray-500">{{ $pastes->total() }} paste{{ $pastes->total() !== 1 ? 's' : '' }} shared</p>
        </div>

        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            @foreach ($pastes as $paste)
                <a href="{{ route('pastes.show', $paste->slug) }}" class="paste-card p-5 block group">
                    {{-- Title --}}
                    ...
                    <p class="mt-2 text-sm text-gray-500 line-clamp-2 font-mono">
                        @if($paste->title)
                            {{ \Illuminate\Support\Str::limit($paste->content, 80) }}
                        @else
                            {{ \Illuminate\Support\Str::limit($paste->content, 80) }}
                        @endif
                    </p>
    ...
    @else
        ...
        <div class="text-center py-16">
            ...
            <div class="mt-6">
                <a href="{{ route('pastes.create') }}" class="btn btn-primary">
