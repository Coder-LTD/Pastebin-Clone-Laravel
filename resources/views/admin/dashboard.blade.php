...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('pastes.index') }}" class="text-sm text-gray-500 hover:text-gray-700 transition-colors flex items-center gap-1">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            ...
            <p class="text-2xl font-bold text-green-600 mt-1">{{ $stats['public'] }}</p>
        </div>
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <p class="text-sm font-medium text-gray-500">Private</p>
            <p class="text-2xl font-bold text-yellow-600 mt-1">{{ $stats['private'] }}</p>
        </div>
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <p class="text-sm font-medium text-gray-500">Unlisted</p>
            <p class="text-2xl font-bold text-purple-600 mt-1">{{ $stats['unlisted'] }}</p>
        </div>
    </div>

    {{-- Filters --}}
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
        <form action="{{ route('admin.dashboard') }}" method="GET" class="flex flex-col sm:flex-row gap-3 items-end">
            <div class="flex-1">
            ...
            <div>
                ...
                >
                    ...
                    <option value="burned" {{ $status === 'burned' ? 'selected' : '' }}>Burned</option>
                </select>
            </div>
            <div class="flex gap-2">
                ...
                @if ($visibility !== 'all' || $status !== 'all' || $search)
                    <a href="{{ route('admin.dashboard') }}" class="rounded-md bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-200 transition-colors inline-flex items-center">
                        Clear
                    </a>
                @endif
            </div>
    ...
    <div class="flex justify-end">
        <form action="{{ route('admin.pastes.expired.delete') }}" method="POST" onsubmit="return confirm('Are you sure you want to delete ALL expired and burned pastes? This action cannot be undone.');">
            @csrf
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <a
                                        ...
                                        title="View paste"
                                    >
                                        {{ \Illuminate\Support\Str::limit($paste->slug, 10) }}
                                    </a>
                                </td>

                                {{-- Title --}}
                                <td class="px-4 py-3 max-w-[200px]">
                                    <span class="text-sm text-gray-900 truncate block" title="{{ $paste->title }}">
                                        {{ \Illuminate\Support\Str::limit($paste->title ?: '(untitled)', 40) }}
                                    </span>
                                </td>
                                ...
                                {{-- Visibility --}}
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <span class="badge badge-{{ $paste->visibility }} text-xs">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap">
                                    ...
                                    @elseif ($paste->expires_at)
                                        <span class="inline-flex items-center gap-1 rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-800" title="{{ $paste->expires_at->toDateTimeString() }}">
                                            {{ $paste->expires_at->diffForHumans(null, true) }}
                                        </span>
                                    @else
                                ...
                                {{-- Created --}}
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500" title="{{ $paste->created_at->toDateTimeString() }}">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-right">
                                    <form
                                        action="{{ route('admin.pastes.destroy', $paste->slug) }}"
                                        method="POST"
                                        onsubmit="return confirm('Delete paste \'{{ addslashes($paste->slug) }}\'? This action cannot be undone.');"
                                        class="inline"
                                    >
                                        @csrf
                                        @method('DELETE')
